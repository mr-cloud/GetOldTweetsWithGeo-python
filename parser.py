        ...
        with open(os.path.join(data_dir, file)) as tweets:
            ...
            for line in tweets:
                ...
                try:
                    ...
                    after_date = line.index(DELIMITER, before_text + len(DELIMITER))
                    dateStr = line[before_text + len(DELIMITER), after_text]
                ...
                time = rst[dateStr]
                if rst[dateStr] is None:
                    time = {}
                geo = time[geoStr]
                if time[geoStr] is None:
                    geo = []
                total = time['all']
                if time['all']:
                    total = []
