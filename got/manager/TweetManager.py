...
class TweetManager:
    ...
    @staticmethod
    def getHtmlResponse(link, cookieJar, prefix='https://twitter.com', locale='US_en', proxy=None):
        url = prefix + link + '?locale=' + locale
        ...
        try:
            ...
            print "Twitter weird response. Try to see on browser: %s" % urllib.quote(
                url)
            # sys.exit()
            return
    ...
    def getTweets(tweetCriteria, receiveBuffer=None, bufferLength=100, proxy=None, BLACK_LIST=['timessquarebomb']):
        ...
        while active:
            ...
            for tweetHTML in tweets:
                ...
                status_page = TweetManager.getHtmlResponse(permalink, cookieJar)
                statPQ = PyQuery(status_page)
                geo = ''
                if statPQ("a.js-geo-pivot-link"):
                    geo = statPQ("a.js-geo-pivot-link").text()
                # geo = ''
